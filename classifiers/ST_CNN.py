...
class Classifier_ST_CNN:
    def __init__(self, output_directory, input_shape, nb_classes, verbose=False):
        if verbose:
            print('Creating ST_CNN Classifier')
    ...
    def build_model(self, input_shape, nb_classes):
        ...
        X = Conv2D(64, (1, input_shape[1]), strides=1, padding="valid", kernel_initializer='he_uniform')(X_input)
        X = keras.layers.BatchNormalization()(X)
        X = keras.layers.ELU(alpha=1.0)(X)

        # Temporal Convolutions
        X = Conv2D(64, (8, 1), strides=1, padding="same", kernel_initializer='he_uniform')(X)
        X = keras.layers.BatchNormalization()(X)
        X = keras.layers.ELU(alpha=1.0)(X)
    ...
    def fit(self, Ximg_train, yimg_train, Ximg_val, yimg_val, epochs, batch_size):
        if self.verbose:
            print('[ST_CNN] Training ST_CNN Classifier')
        ...
        print('[ST_CNN] Training done!, took {}s'.format(self.duration))
