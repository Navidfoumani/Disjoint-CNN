...
import keras
from keras.layers import Input, Conv2D, BatchNormalization, ELU, Reshape, Permute, MaxPooling2D
...
class Classifier_DCNN_4L:
    ...
    def build_model(self, input_shape, nb_classes):
        ...
        gap_layer = keras.layers.pooling.GlobalAveragePooling2D()(MaxPool)
    ...
    def fit(self, Ximg_train, yimg_train, Ximg_val, yimg_val, epochs, batch_size):
        if self.verbose:
            print('[DCNN_4L] Training DCNN_4L Classifier')
        ...
        if self.verbose:
            print('[DCNN_4L] Training done!, took {}s'.format(self.duration))
        return

    def predict(self, X_img, y_img, best):
        if self.verbose:
            print('[DCNN_4L] Predicting')
        if best:
