...
class Classifier_S_CNN:
    def __init__(self, output_directory, input_shape, nb_classes, verbose=False):
        if verbose:
            print('Creating S_CNN Classifier')
    ...
    def build_model(self, input_shape, nb_classes):
        ...
        X = MaxPooling2D(pool_size=(5, 1), strides=None, padding='valid')(X)
        gap_DCNN = GlobalAveragePooling2D()(X)
    ...
    def fit(self, Ximg_train, yimg_train, Ximg_val, yimg_val, epochs, batch_size):
        if self.verbose:
            print('[S_CNN] Training S_CNN Classifier')
        ...
        print('[S_CNN] Training done!, took {}s'.format(self.duration))
